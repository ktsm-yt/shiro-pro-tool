...
## 🆕 最新状況（2025-11-20）
- `js/app.js` に `fetchFromWikiURL` を実装済み。`corsproxy.io` → `api.codetabs.com` → `api.allorigins.win` の順にフェールオーバーしてHTMLを取得。
- 取得したHTMLから **名前/期間プレフィックス、武器種、遠近・物術・配置、属性** を抽出し、フォームへ自動入力。
- 特技・計略テキストを分割表示し、各テキストの「解析」ボタンで正規表現ベースのバフ抽出を行い、リストへ直接追加可能（半自動解析）。
- 解析ロジック・インポートUIは `index.html` に組込み済み。データはLocalStorageに保存される。
- CORSエラーの場合は失敗を明示し、ローカルサーバー/拡張の利用を促すメッセージを表示。

---

## 🎯 実装フェーズ

### Phase 1: 基本情報の自動入力 ⭐ **完了**
...
#### 1.2 データ取得
- **CORS対策**: プロキシサービスを使用
  - 現行: `https://corsproxy.io/?` → `https://api.codetabs.com/v1/proxy?quest=` → `https://api.allorigins.win/raw?url=`
- DOMParserでHTMLをパース
...
現状: URL取得後に表示される特技/計略テキストの「解析」ボタンを押した時にこの検出が走り、検出結果をバフリストへ追加する。URL貼付だけでの自動解析は未実装。
...
### Phase 1（必須）
- [x] WikiのURLを入力して「取得」ボタンをクリック
- [x] 名前、武器種、属性、遠近、物術、配置が自動入力される
- [x] エラー時に適切なメッセージが表示される
- [x] 提供された3つのURLで正常動作する（CORS失敗時は明示的に案内）

### Phase 2（推奨）
- [x] 特技・計略のテキストが参照可能
- [x] ユーザーが見ながら手動入力できる（解析ボタンで半自動入力も可）

### Phase 3（オプション）
- [~] 単純なバフパターンが自動検出される（解析ボタンで手動トリガーは実装済み）
- [x] 検出されたバフがバフリストに追加される
- [x] ユーザーが確認・修正できる
...
## 🗓 更新履歴
- 2025-11-20: 現状反映（Phase1/2完了、Phase3を手動トリガー実装と明記、成功基準更新）
- 2025-10-??: 初版草案
